...
def relationscutter_path() -> Path:
    ...
    #if (dsharpy_base.parent / "cbmc").exists():  # we're clearly in my current debug setup
    #    return dsharpy_base.parent / "cbmc/build/bin/cbmc"
    return next(f for f in (dsharpy_base / "util" / "relationscutter" / "build").rglob("relationscutter") if f.is_file()).absolute()
...
def run_cbmc(c_file: Union[Path, str], cnf_file: Path, options: CBMCOptions = None, cbmc_path: Path = modified_cbmc_path(),
    ...
    cmd = [str(cbmc_path), str(c_file), f"--{options.int_width}", "--unwind", str(options.unwind), "--dimacs"] + (misc or [])
    if verbose:
        print(" ".join(cmd))
...
def process_with_options(code: Union[Path, str], cbmc_options: CBMCOptions = None, cutter_options: RelationsCutterOptions = None) -> int:
    with tempfile.TemporaryDirectory() as tmpfolder:
        cnf_file = (process_path_with_cbmc if isinstance(code, Path) else process_code_with_cbmc)(code, Path(tmpfolder), cbmc_options)
...
def process(code: Union[Path, str], real_unwind: int = 2, int_width: int = 32, verbose: bool = False) -> int:
    return process_with_options(code, CBMCOptions(unwind=real_unwind + 2, rec=real_unwind-1, int_width=int_width, verbose=verbose),
